...
impl<T: PrimeField> BatchVerifier<T> {
    ...
    pub fn verify(&self, (interpolation_initial_proof, interpolation_proof): &(QueryVecsResult<T>, Vec<QueryResult<T>>), evaluations: &Vec<T>) -> bool {
        let mut leaf_indices = self.oracle.query_list.clone();
        for i in 0..self.total_round {
            let domain_size = self.interpolate_cosets[i].size();
            leaf_indices = leaf_indices
            ...
            if i == 0 {
                interpolation_initial_proof.verify_merkle_tree(&leaf_indices, &self.interpolation_initial_root);
            } else {
                interpolation_proof[i - 1].verify_merkle_tree(&leaf_indices, &self.interpolation_roots[i - 1]);
            }
            ...
            let get_folding_value: Box<dyn Fn(&usize) -> T> = if i == 0 {
                Box::new(|x| {
                    ...
                    for j in 0..vec.len() {
                        let value = (vec[j] - evaluations[j]) * (self.interpolate_cosets[0].element(*x) - self.open_points[j]).inverse().unwrap();
