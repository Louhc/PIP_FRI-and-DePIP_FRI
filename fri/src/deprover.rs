...
pub struct DeProver<T: PrimeField> {
    ...
    de_total_round: usize,
    sub_polynomial: UnivariatePolynomial<T>,
    // cosets of each round, each sub-prover holds the same
    de_interpolate_cosets: Vec<GeneralEvaluationDomain<T>>,
...
impl<T: PrimeField> DeProver<T> {
    pub fn new(
        ...
        let interpolate_polynomial =
            ComOpenOneVec::new(interpolate_coset[0].fft(&sub_polynomial.coeffs));
        DeProver {
            sub_prover_id,
            total_round,
            sub_polynomial,
