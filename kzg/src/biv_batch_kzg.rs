...
use crate::helper::{
    divide_by_x_minus_k, evaluate_one_lagrange, interpolate_evaluate_one_no_repeat,
};
use crate::uni_trivial_kzg::{self, DeKZG, KZG};
use crate::Error;
use crate::{
    helper::{generate_powers, generator_numerator_polynomial, interpolate_on_trivial_domain},
...
macro_rules! par_join_3 {
    ($task1:expr, $task2:expr, $task3:expr) => {{
        let ((result1, result2), result3) = rayon::join(|| rayon::join($task1, $task2), $task3);
...
impl<P: Pairing> BivBatchKZG<P> {
    ...
    > {
        Ok(
            BivariateKZG::<P>::setup_double_lagrange(rng, x_degree, y_degree, x_domain, y_domain)
                .unwrap(),
        )
    }
    ...
    pub fn de_commit(
        ...
        let sub_powers =
            &powers[sub_prover_id * x_domain.size()..(sub_prover_id + 1) * x_domain.size()];

        let sub_coms: Vec<P::G1Affine> = sub_polynomials
            .par_iter()
            .map(|polynomial| P::G1MSM::msm_unchecked(&sub_powers, &polynomial.coeffs).into())
            .collect();
    ...
    pub fn de_commit_double_lagrange(
        ...
        let sub_coms: Vec<P::G1Affine> = sub_evals
            .par_iter()
            .map(|evals| P::G1MSM::msm_unchecked(&sub_powers, &evals).into())
            .collect();
    ...
    pub fn de_open_lagrange(
        ...
        let mut polynomial_q1 = sub_polynomials
            .par_iter()
    ...
    pub fn de_open_lagrange_with_eval(
        ...
        if Net::am_master() {
            ...
            let target_evals: Vec<P::ScalarField> = (0..evals_slice.len())
                .into_par_iter()
                .map(|i| {
                    evals
                        .par_iter()
                        .zip(evals_lagrange.par_iter())
                        .map(|(row, eval_lagrange)| row[i] * eval_lagrange)
                        .sum()
                })
                .collect();
    ...
    pub fn de_open_double_lagrange_with_eval(
        ...
        let mut divider_vec: Vec<P::ScalarField> =
            x_domain.elements().map(|element| element - *x).collect();
        ...
        let evals_point: Vec<P::ScalarField> = sub_evals
            .par_iter()
            .map(|evals| {
                evals
                    .par_iter()
                    .zip(evals_x_lagrange.par_iter())
                    .map(|(left, right)| *left * *right)
                    .sum()
            })
            .collect();
        let evals_q1: Vec<P::ScalarField> = (0..x_domain.size())
            .into_par_iter()
            .map(|col_index| {
                sub_evals
                    .par_iter()
                    .zip(evals_point.par_iter())
                    .zip(linear_factors.par_iter())
                    .map(|((evals, eval_x), factor)| (evals[col_index] - *eval_x) * factor)
                    .sum::<P::ScalarField>()
                    * divider_vec[col_index]
            })
            .collect();
        ...
        if Net::am_master() {
            ...
            let target_evals: Vec<P::ScalarField> = (0..evals.len())
                .into_par_iter()
                .map(|i| {
                    evals
                        .par_iter()
            ...
            let coeffs_q2 =
                KZG::<P>::get_quotient_eval_lagrange(&sub_poly_evals_sum, &y, &y_domain);
            let proof_2 = P::G1MSM::msm_unchecked_par_auto(&y_srs, &coeffs_q2);
    ...
    pub fn de_open_lagrange_at_same_y(
        ...
        // generate q_i()
        let eval_lagrange: <P as Pairing>::ScalarField =
            evaluate_one_lagrange::<P>(sub_prover_id, domain, y_point);
        ...
        factors
            .iter_mut()
            .for_each(|factor| *factor *= eval_lagrange);

        let combined_polynomial = results
            .par_iter()
        ...
        let proof_q_slice =
            P::G1MSM::msm_unchecked_par_auto(&x_srs, &polynomial_q_slice.coeffs).into();
        ...
        let sub_evals_eta: Vec<P::ScalarField> = sub_polynomials
            .par_iter()
            .map(|poly| poly.evaluate(&eta))
            .collect();
        let evals_eta_beta: Vec<P::ScalarField> = sub_evals_eta
            .par_iter()
            .map(|eval| eval.clone() * eval_lagrange)
            .collect();
        ...
        let (evals_eta_beta, eval_q) = if Net::am_master() {
            ...
            let eval_q: <P as Pairing>::ScalarField = evals_eta_beta_and_q
                .par_iter()
                .map(|&(_, second)| second)
                .sum();
            let mut evals_eta_beta =
                vec![<P as Pairing>::ScalarField::zero(); evals_eta_beta_and_q[0].0.len()];
            ...
            evals_eta_beta
                ...
                .enumerate()
                .for_each(|(i, eval)| {
                    *eval = evals_eta_beta_and_q
                        .par_iter()
                        .map(|eval_eta_beta_and_q| eval_eta_beta_and_q.0[i])
                        .sum();
                });
        ...
        let proof_q1_q2 = Self::de_open_lagrange(
            sub_prover_id,
            &powers,
            &y_srs,
            &sub_polynomials,
    ...
    pub fn verify_at_same_y(
        ...
        let left_check2: P::ScalarField = x_points
            .par_iter()
            .zip(evals.par_iter())
            .zip(linear_factors.par_iter())
            .zip(proof.1.par_iter())
            .map(|(((points, cur_evals), factor), proof)| {
                ...
                let eval_helper =
                    num_eval / generator_numerator_polynomial::<P>(&points).evaluate(&eta);
    ...
    pub fn verify_at_same_y_optimized(
        ...
        let (check1, check2, check3) = par_join_3!(
            ...
            || {
                ...
                let mut aux_num_evals = vec![
                    eta - alpha,
                    (eta - r_alpha) * (eta - r),
                    (eta - alpha) * (eta - r_inv) * eta,
                    eta - r,
                    eta - r_pow_m,
                ];
    ...
    pub fn verify(
        ...
        let (left, right1, right2) = par_join_3!(
            || {
                ...
                let extra_scalar = -evals
                    .iter()
                    .zip(challenge_vector.iter())
                    .map(|(eval, factor)| *eval * factor)
                    .sum::<P::ScalarField>();
                ...
                let linear_combination =
                    P::G1MSM::msm_unchecked_par_auto(&bases, &challenge_vector);
    ...
    pub fn commit(
        ...
        let coms: Vec<P::G1> = bivariate_polynomials
            .par_iter()
            .map(|biv_poly| {
                biv_poly
                    .x_polynomials
                    .par_iter()
                    .zip(powers.par_chunks_exact(x_size))
                    .map(|(poly, power)| -> P::G1 {
    ...
    pub fn open(
        ...
        for j in 0..bivariate_polynomials.len() {
            let evals: Vec<P::ScalarField> = bivariate_polynomials[j]
                .x_polynomials
                .iter()
            ...
            let combined_polynomial_slice_q2 =
                &UnivariatePolynomial::from_coefficients_vec(evals) * linear_factor;
        ...
        for i in 0..y_srs.len() {
            ...
            for j in 0..bivariate_polynomials.len() {
                combined_polynomial_slice_for_q1 +=
                    &(bivariate_polynomials[j].x_polynomials[i] * linear_factor);
                linear_factor *= challenge;
            }
        ...
        let proof = (
            P::G1MSM::msm_unchecked_par_auto(&powers, &coeffs_q1)
                .into()
                .into(),
            P::G1MSM::msm_unchecked_par_auto(&y_srs, &combined_polynomial_q2.coeffs)
                .into()
                .into(),
        );
    ...
    pub fn open_lagrange(
        ...
        // ]);
        let coeffs_q2 =
            KZG::<P>::get_quotient_eval_lagrange(&combined_polynomial_q2.coeffs, &y, &domain);
        ...
        let proof = (
            P::G1MSM::msm_unchecked_par_auto(&powers, &coeffs_q1)
                .into()
                .into(),
            P::G1MSM::msm_unchecked_par_auto(&y_srs, &coeffs_q2)
                .into()
                .into(),
        );
    ...
    > {
        ...
        for i in 0..x_points.len() {
            ...
            for j in 0..x_points[i].len() {
                let point: (<P as Pairing>::ScalarField, <P as Pairing>::ScalarField) =
                    (x_points[i][j], y_point.clone());
            ...
            let helper_polynomial =
                &numerator_polynomial / &(generator_numerator_polynomial::<P>(&x_points[i]));
            let combined_polynomial_slice =
                &(&polynomial_f_x_beta - &polynomial_r) * &helper_polynomial;
        ...
        let proof_q =
            P::G1MSM::msm_unchecked_par_auto(&powers[..x_domain.size()], &polynomial_q.coeffs)
                .into()
                .into();
        ...
        let evals_eta_beta: Vec<P::ScalarField> = bivariate_polynomials
            .iter()
            .map(|poly| poly.evaluate(&point_eta_beta))
            .collect();
    ...
    > {
        ...
        for i in 0..x_points.len() {
            ...
            let mut polynomial_f_x_beta = UnivariatePolynomial::zero();
            let evals_y_lagrange =
                EvaluationDomain::evaluate_all_lagrange_coefficients(domain, *y_point);
            for k in 0..bivariate_polynomials[i].x_polynomials.len() {
                polynomial_f_x_beta += (
                    evals_y_lagrange[k],
                    bivariate_polynomials[i].x_polynomials[k],
                );
            }
            ...
            let combined_polynomial_slice =
                &polynomial_f_x_beta / &(generator_numerator_polynomial::<P>(&x_points[i]));
            combined_polynomial += (challenge_gamma, &combined_polynomial_slice);
            challenge_gamma *= gamma;
        }
        ...
        let time = Instant::now();
        let proof_q = P::G1MSM::msm_unchecked_par_auto(&x_srs, &polynomial_q.coeffs)
            .into()
            .into();
        ...
        let evals_eta_beta: Vec<P::ScalarField> = bivariate_polynomials
            .iter()
            .map(|poly| poly.evaluate_lagrange(&point_eta_beta, &domain))
            .collect();
        let eval_q_eta: P::ScalarField = polynomial_q.evaluate(&eta);
        // let proof_eval = (evals_eta_beta, eval_q_eta);
        ...
        let time = Instant::now();
        let proof_q1_q2 = Self::open_lagrange(
            &powers,
            y_srs,
            &bivariate_polynomials,
            &point_eta_beta,
...
mod tests {
        ...
        for i in 0..POLYNOMIAL_NUMBER {
            bivariate_polynomials.push(BivariatePolynomial {
                x_polynomials: &x_poly_refs
                    [i * (BIVARIATE_Y_DEGREE + 1)..(i + 1) * (BIVARIATE_Y_DEGREE + 1)],
            });
        }

        // Commit to the polynomials
        let coms = TestBivariatePolyCommitment::commit(&xy_srs, &bivariate_polynomials).unwrap();
        let mut prover_transcript: Transcript =
            Transcript::new(b"batch bivariate KZG at the same point");
        let gamma = <Transcript as ProofTranscript<Bls12_381>>::challenge_scalar(
            &mut prover_transcript,
            b"combined_polynomials_evaluated_at_the_same_point",
        );
    ...
    fn bivariate_poly_commit_lagrange_at_the_same_point_test() {
        ...
        let domain =
            ...
            >>::new(BIVARIATE_Y_DEGREE + 1)
            .unwrap();
        let ((xy_srs, _, y_srs), verifier) = TestBivariatePolyCommitment::setup_lagrange(
            &mut rng,
            BIVARIATE_X_DEGREE,
            BIVARIATE_Y_DEGREE,
            &domain,
    ...
    fn bivariate_poly_commit_at_the_same_y_test() {
        ...
        // proof size
        let proof_size = (eval_proof.1.len() + 1)
            * size_of_val(&<Bls12_381 as Pairing>::ScalarField::one())
            + 4 * size_of_val(&eval_proof.0);
        ...
        assert!(TestBivariatePolyCommitment::verify_at_same_y(
            &verifier,
            &coms,
            &x_points,
            &y_point,
    ...
    fn bivariate_poly_commit_lagrange_at_the_same_y_test() {
        ...
        let domain =
            ...
            >>::new(BIVARIATE_Y_DEGREE + 1)
            .unwrap();
        let ((xy_srs, x_srs, y_srs), verifier) = TestBivariatePolyCommitment::setup_lagrange(
            &mut rng,
            BIVARIATE_X_DEGREE,
            BIVARIATE_Y_DEGREE,
            &domain,
