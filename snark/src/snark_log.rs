...
impl<P: Pairing> DeSNARKLog<P> {
    ...
    pub fn de_r1cs_prove (
        ...
        let proofs_wit_upper_r_polys= BivBatchKZG::<P>::de_open_lagrange_at_same_y(sub_prover_id, &powers, &x_srs, &y_srs, &sub_polynomials, &x_points, &beta, &y_domain, transcript, &gamma);
        end_timer!(step);
        ...
        let (evals_f1, proof_f1) = PreProver::<P>::open_f1(sub_prover_id, &m_powers, &m_y_srs, &de_polys_f1, &precombined_f1, &y_domain, &gamma);
        end_timer!(step);

        // open L at (beta, zeta)
        // the prover doesn't need to compute com to L(X, Y) as it is generated by indexer
        let step = start_timer!(|| "open L at (beta, zeta)");
        let (eval_l, proof_l) = PreProver::<P>::open_l_beta_zeta(sub_prover_id, &powers, &y_srs, &de_poly_l, &eval_beta, &y_domain, &beta, &zeta, &gamma);
        if Net::am_master() {
