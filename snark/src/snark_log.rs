...
impl<P: Pairing> DeSNARKLog<P> {
    ...
    pub fn de_r1cs_prove (
        ...
        let proofs_wit_upper_r_polys= BivBatchKZG::<P>::de_open_lagrange_at_same_y(sub_prover_id, &powers, &x_srs, &y_srs, &sub_polynomials, &x_points, &beta, x_domain, &y_domain, transcript, &gamma);
        end_timer!(step);
        ...
        let (eval_l, proof_l) = PreProver::<P>::open_l_beta_zeta(sub_prover_id, &powers, &y_srs, &de_poly_l, &eval_beta, x_domain, &y_domain, &beta, &zeta, &gamma);
        if Net::am_master() {
