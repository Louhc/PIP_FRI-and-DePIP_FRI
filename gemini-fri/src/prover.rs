...
#[derive(Clone)]
// evaluations are the pairs of opened points
pub struct Function<T: PrimeField> {
    interpolation: InterpolateValue<T>,
    evaluations: Vec<(T, T)>,
}

impl<T: PrimeField> Function<T> {
...
// function, polynomial, folding are all vectors
#[derive(Clone)]
...
impl<T: PrimeField> FriProver<T> {
    ...
    // compute the evaluations of tuples
    pub fn compute_tuples(&mut self) -> Vec<Tuple<T>> {
        let beta = self.oracle.beta;
        let mut tuples = vec![];
        for i in 0..self.total_round {
            tuples.push(Tuple {
                a: self.polynomials[i].evaluate_as_uni_polynomial(beta),
                b: self.polynomials[i].evaluate_as_uni_polynomial(-beta),
                c: self.polynomials[i + 1].evaluate_as_uni_polynomial(beta * beta),
            });
        }
    ...
    // generate and push functions, the log N new polynomials to be committed
    pub fn commit_functions(&mut self, verifier: &mut FriVerifier<T>, open_point: &Vec<T>) {
        ...
        // all polynomials run fft on the initial domain, and can't aggregate the entries into one Merkle tree
        // the log N polynomials merkle tree roots
    ...
    fn initial_interpolation(&self) -> Vec<T> {
        ...
        for i in 0..self.functions.len() {
            ...
            for (x, y) in &self.functions[i].evaluations {
                let mut v: Vec<T> = self.interpolate_cosets[0].elements()
                    // .all_elements()
                    .into_iter()
                    .map(|i| i - *x)
                    .collect();
                ...
                for j in 0..interpolation.len() {
                    res[j] += v[j] * (interpolation[j] - *y) * acc;
                }
                acc *= rlc
            }
        }
    ...
    pub fn open(&mut self, verifier: &mut FriVerifier<T>, point: &Vec<T>) -> (Vec<Tuple<T>>, Vec<QueryResult<T>>, Vec<QueryResult<T>>)
    {
        self.commit_functions(verifier, &point);
        let tuples = self.compute_tuples();
        self.prove();
        self.commit_foldings(verifier);
        let (folding_proofs, function_proofs) = self.query();
        (tuples, folding_proofs, function_proofs)
    }
}
